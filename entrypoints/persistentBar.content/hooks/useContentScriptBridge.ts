...
export function useContentScriptBridge(dispatch: React.Dispatch<BarAction>) {
  useEffect(() => {
    console.log('[useContentScriptBridge] Setting up event listeners...');

    // Listen for scroll count updates
    const handleScrollUpdate = (event: CustomEvent) => {
      const { count } = event.detail;
      console.log('[useContentScriptBridge] Scroll count updated:', count);
      dispatch({
    ...
    const handleLockedUpdate = (event: CustomEvent) => {
      const { count } = event.detail;
      console.log('[useContentScriptBridge] Locked count updated:', count);
      dispatch({
    ...
    const handleTrackingChange = (event: CustomEvent) => {
      ...
      console.log('[useContentScriptBridge] Tracking state changed:', isTracking);
      dispatch({
        type: 'SET_TRACKING',
        payload: isTracking,
      });
    ...
    const handleInteractiveModeChange = (event: CustomEvent) => {
      ...
      console.log('[useContentScriptBridge] Interactive mode changed:', isInteractive);
      dispatch({
    ...
    return () => {
      console.log('[useContentScriptBridge] Cleaning up event listeners...');
